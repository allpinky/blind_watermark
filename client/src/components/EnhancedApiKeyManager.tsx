...
export default function EnhancedApiKeyManager() {
  ...
  const testAllKeys = async (provider: string, keys: any[]) => {
    let successCount = 0;
    let failCount = 0;

    for (const key of keys) {
      try {
        const response = await fetch(`/api/admin/apikeys/${provider}/test`, {
          method: 'POST',
                ...
                <CardHeader className="pb-3">
                  <div className="flex items-center justify-between">
                    ...
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => testAllKeys(provider, stats?.keys || [])}
                    >
                                          ...
                                          <div>
                                            ...
                                            <div className="flex items-center gap-2 p-2">
                                              {editingKey.isActive ? (
                                                ...
                                                <><XCircle className="w-4 h-4 text-red-500" /> Ngưng hoạt động</>
                                              )}
                                            </div>
                                          </div>
                                        </div>
